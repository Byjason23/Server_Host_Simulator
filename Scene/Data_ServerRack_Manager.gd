...
func updateRack():
	var index = Server_Upgrade.updates_bought_serverrack
	if index >= 5:
		index = 5
	createServerRack(ServerRack_array[index])
...
func _on_save_marker_load_data_rack(data_Server_Rack):
	setserverupgrade(data_Server_Rack)


func _on_upgrade_ui_upgrade_data_rack(Rack_Data: Variant) -> void:
